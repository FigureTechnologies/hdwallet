...
public class Base58 {
    ...
    public static String encodeChecked(byte[] payload) {
        ...
        byte[] checksum = HashKt.hashTwice(addressBytes, 0, payload.length);
    ...
    public static byte[] decodeChecked(String input) throws InvalidDataLength, InvalidChecksum, InvalidCharacter {
        ...
        if (decoded.length < 4)
            throw new InvalidDataLength("Input too short: " + decoded.length);
        byte[] data = Arrays.copyOfRange(decoded, 0, decoded.length - 4);
        byte[] checksum = Arrays.copyOfRange(decoded, decoded.length - 4, decoded.length);
        byte[] actualChecksum = Arrays.copyOfRange(HashKt.sha256(HashKt.sha256(data)), 0, 4);
